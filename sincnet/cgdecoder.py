...
class FrameInverseCGFunction(torch.autograd.Function):
    ...
    @staticmethod
    def _cg_solve(normal_op:Callable[[torch.Tensor], torch.Tensor], b:torch.Tensor,
                  n_iter:int=64, tol:float=1e-9, eps:float=1e-30) -> torch.Tensor:
        ...
        for _ in range(n_iter):
            ...
            if float(next_norm.sqrt().max()) < tol:
                break
            direction = residual + (next_norm / residual_norm.clamp_min(eps)) * direction
            residual_norm = next_norm
        return x

    @staticmethod
    def _adjoint_op(encoder:nn.Module, batch_size:int, length:int, device:torch.device,
    ...
    @staticmethod
    def forward(ctx:torch.autograd.function.FunctionCtx, spec:torch.Tensor, encoder:nn.Module,
                length:int, n_iter:int, tol:float, reg:float) -> torch.Tensor:
        adjoint = FrameInverseCGFunction._adjoint_op(encoder, spec.shape[0], length, spec.device, spec.dtype)
        ...
        ctx.reg = reg
        with torch.no_grad():
            return FrameInverseCGFunction._cg_solve(normal_op, adjoint(spec), n_iter=n_iter, tol=tol)

    @staticmethod
    def backward(ctx:torch.autograd.function.FunctionCtx, grad_x:torch.Tensor) -> tuple[torch.Tensor, None, None, None, None, None]:
        encoder: nn.Module = ctx.encoder
        length: int = ctx.length
        reg: float = ctx.reg
        adjoint = FrameInverseCGFunction._adjoint_op(encoder, grad_x.shape[0], length,
                                                     grad_x.device, grad_x.dtype)
        normal_op: Callable[[torch.Tensor], torch.Tensor] = (
            (lambda value: adjoint(encoder(value)) + reg * value)
            if reg
            else (lambda value: adjoint(encoder(value)))
        ...
        with torch.no_grad():
            solution = FrameInverseCGFunction._cg_solve(normal_op, grad_x.contiguous(),
                                                        n_iter=ctx.n_iter, tol=ctx.tol)
            grad_spec = encoder(solution)
        return grad_spec, None, None, None, None, None


def frame_pseudo_inverse(spec:torch.Tensor, encoder:nn.Module, length:int, n_iter:int=64,
                         tol:float=1e-9, reg:float=0.0) -> torch.Tensor:
